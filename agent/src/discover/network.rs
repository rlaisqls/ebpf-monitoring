...
pub async fn get_networks_labels(
	...
	let networks = match client.networks().list(&Default::default()).await {
		...
		Err(e) => Err(OSError(format!("error while listing networks: {}", e.to_string())))
	}.unwrap();
	...
	for network in networks {
		...
		network_labels.insert(format!("{}{}", label_prefix, LABEL_NETWORK_INTERNAL), network.internal.unwrap().to_string());
		network_labels.insert(format!("{}{}", label_prefix, LABEL_NETWORK_INGRESS), network.ingress.unwrap().to_string());
